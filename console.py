...
class HBNBCommand(cmd.Cmd):
    ...
    def do_destroy(self, line):
        """This command deletes an instance based on the class name & id."""
        command = self.parseline(line)[0]
        arg = self.parseline(line)[1]
        if command is None:
            print('** class name missing **')
    ...
    def do_all(self, line):
        """This command prints all string representation of all
        instances based or not in the class name.
        """
        command = self.parseline(line)[0]
    ...
    def analyze_parameter_value(self, line):
        ...
        Analyze if a parameter is a string that needs to convert
        to a float number or an integer number.

        Args:
            value: A value to analyze
        """
        if value.isdigit():
            return int(value)
    ...
    def get_objects(self, instance=''):
        ...
        This function obtains the information of all the instances
        created in the file `objects.json` which is used as the
        storage engine.

        When an instance is sent as an argument, the function takes
        care of the matched instance only.

        Args:
            instance (:obj:`str`, optional): The instance to find the
            objects.

        Return:
            list: If an `instance` argument is not empty, it search only
            for objects that match the instance. Otherwise, it will show
            all instances in the file where objects are stored.
        """
    ...
    def default(self, line):
        """When a command prefix is not recognized, this function
        looks if the command entered has the syntax:
        "<class name>.<method name>" or not and links it to the
        corresponding method in case the class exists and the method
